...
def build_oci_command(command: str, profile: str) -> list[str]:
    """Build the OCI CLI command, selecting auth mode from the active profile."""
    cli_command = ["oci", "--profile", profile]
    config_path = os.getenv("OCI_CONFIG_FILE", oci.config.DEFAULT_LOCATION)
    parser = ConfigParser()

    try:
        read_files = parser.read(config_path)
        if read_files and parser.has_option(profile, "security_token_file"):
            cli_command.extend(["--auth", "security_token"])
    except Exception as exc:  # pragma: no cover - fall back to default CLI behavior
        logger.warning(f"Unable to inspect OCI config file '{config_path}': {exc}")
...
def run_oci_command(
    ...
    # Run OCI CLI command using subprocess
    try:
        cli_command = build_oci_command(command, profile)
        result = subprocess.run(
            cli_command,
            env=env_copy,
            capture_output=True,
